  ...
  if (convInfo.filterHeight === 1 && convInfo.filterWidth === 1 &&
    ...
    const grad = (dy: Tensor4D) => {
      util.assert(
          ...
          () =>
              'Error in gradient of conv2D: dilation rates greater than 1 are' +
...
function depthwiseConv2d_<T extends Tensor3D|Tensor4D>(
  ...
  const grad = (dy: Tensor4D) => {
    util.assert(
        conv_util.tupleValuesAreOne(dilations),
        () => 'Error in gradient of depthwiseConv2d: dilation rates greater ' +
            `than 1 are not yet supported. Got dilations '${dilations}'`);
    return {
