...
function batchToSpaceND_<T extends Tensor>(
  ...
  util.assert(
      ...
      () => `crops.length is ${crops.length} but should be equal to ` +
          `blockShape.length  ${blockShape.length}`);

  util.assert(
      $x.shape[0] % prod === 0,
      () => `input tensor batch is ${$x.shape[0]} but is not divisible ` +
          `by the product of the elements of blockShape ` +
...
function spaceToBatchND_<T extends Tensor>(
  ...
  util.assert(
      ...
      () => `paddings.shape[0] ${paddings.length} must be equal to ` +
...
function depthToSpace_(
  ...
  util.assert(
      ...
      () => `Negative dimension size caused by overflow when multiplying ` +
          `${inputHeight} and ${blockSize}  for depthToSpace with input ` +
          `shape ${$x.shape}`);

  util.assert(
      inputWidth * blockSize >= 0,
      () => `Negative dimension size caused by overflow when multiplying ` +
          `${inputWidth} and ${blockSize} for depthToSpace with input ` +
          `shape ${$x.shape}`);

  util.assert(
      (inputDepth % (blockSize * blockSize) === 0),
      () => `Dimension size must be evenly divisible by ` +
          `${blockSize * blockSize} but is ${inputDepth} for depthToSpace ` +
          `with input shape ${$x.shape}`);
...
async function setdiff1dAsync_(
  ...
  util.assert(
      $x.dtype === $y.dtype,
      () => `x and y should have the same dtype, but got x (${$x.dtype}) ` +
          `and y (${$y.dtype}).`);
